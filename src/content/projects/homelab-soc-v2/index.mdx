...
import ArchitectureDiagramInteractive from '../../../components/interactive/ArchitectureDiagramInteractive.astro';
import Terminal from '../../../components/interactive/Terminal.astro';
import DashboardGallery from '../../../components/projects/DashboardGallery.astro';
import DashboardShowcase from '../../../components/projects/DashboardShowcase.astro';
import EvolutionTimeline from '../../../components/interactive/EvolutionTimeline.astro';
...
<DashboardShowcase 
  src="/images/projects/homelab-soc-v2/dashboard-soc-overview.png"
  alt="SOC Overview Dashboard - Real-time security monitoring"
  caption="Live SOC Dashboard — 51K+ alerts, 15 countries, real-time threat detection"
/>
...
<StatsCounter stats={[
  ...
  { value: 2000, suffix: "+", label: "IPs Auto-Blocked" }
]} />
...
This SOC monitors two production websites—**brianchaplow.com** and **bytesbourbonbbq.com**—in real-time. Real visitors. Real bots. Real attackers. When someone probes my sites from Romania or runs an Nmap scan from India, I see it, enrich it with threat intelligence, and automatically block them at the edge.
...
Nine Docker containers on the QNAP handle detection, analysis, and response:
...
<div class="not-prose grid grid-cols-2 gap-4 my-8">
  ...
  <div class="group text-center p-5 rounded-xl bg-gradient-to-br from-white/5 to-transparent border border-white/10 hover:border-cyan-500/30 transition-all duration-300">
    ...
    <div class="text-sm text-neutral-500 mt-1">47,487 IDS Rules</div>
  </div>
  <div class="group text-center p-5 rounded-xl bg-gradient-to-br from-white/5 to-transparent border border-white/10 hover:border-cyan-500/30 transition-all duration-300">
...
<DashboardGallery dashboards={[
  {
    src: "/images/projects/homelab-soc-v2/dashboard-nids-detection.png",
    ...
    highlight: "47,487 active rules"
  },
  {
    src: "/images/projects/homelab-soc-v2/dashboard-windows-security.png",
  ...
  {
    src: "/images/projects/homelab-soc-v2/dashboard-threat-intel.png",
...
With Kali on VLAN 20 and Metasploitable on VLAN 40, I run real exploits and verify detection. Here's CVE-2011-2523 — the vsftpd 2.3.4 backdoor:

<Terminal 
  title="butcher@sear — msfconsole"
  promptUser="butcher"
  promptHost="sear"
...
- Shell session 10.10.20.20 → 10.10.40.20:6200 logged
- Full attack timeline in OpenSearch
- Target isolated to VLAN 40 — no lateral movement possible
...
<EvolutionTimeline items={[
  ...
  { before: "~100 IPs blocked", after: "2,000+ IPs blocked", improvement: "20x volume" }
]} />

Each limitation in v1 became a learning opportunity. The progression shows continuous improvement—exactly what employers want to see.

<div class="not-prose mt-6 p-4 rounded-lg bg-white/5 border border-white/10">
  <p class="text-sm text-neutral-400"><strong class="text-neutral-300">📁 See where it started:</strong> <a href="/projects/homelab-soc-v1/" class="text-cyan-400 hover:text-cyan-300 underline">HomeLab SOC v1</a> — The original architecture before VLAN segmentation and hardware upgrades.</p>
</div>
